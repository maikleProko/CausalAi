...
export default function MakeCausalInferenceButton(props) {

    function makeResearch(csv) {
        let data = getCausalSource(csv)
        fetch(`http://127.0.0.1:5000/get_causal_inference_ate`, {
            method: "POST",
            headers: {
            ...
            .then((data) => {
                    props.handler(data.estimate_ate)
                }
            );
    }
    ...
    function getCausalSource(csv) {
        ...
        return {
            ...
            'graph': props.graph,
            'target_variable': props.targetVariable,
            'treatment_variables': props.treatmentVariables,
            'prediction_model': props.algorithm,
            'random_seed': 0.5,
            'num_vars': getVariables(csv, splitSign).length,
            'num_samples': 0.05,
            'is_discrete': true
        }
    }
    ...
    useEffect(() => {
        console.log(removeCommas(" 123123123 \"123, 123 ,,\" ,"))
    ...
    return (
        <>
            {showButton(props.variables)}
        </>
    )

    MakeCausalInferenceButton.propTypes = {
        ...
        treatmentVariables: PropTypes.array
    }
}
